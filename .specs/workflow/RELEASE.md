# RELEASE — `sistema-sindico`

Processo para cortar uma release de `sistema-sindico` (`TBD`, stack `dotnet`). Releases são tagueadas, automatizadas via GitHub Actions e reversíveis. Dono do processo: `Wesley Simplicio`.
  ...
  deploy:
    ...
    steps:
      ...
      - name: Rollout production
        run: <comando rollout>
      - name: Smoke test
        run: npm run test:smoke -- --baseUrl=https://sistema-sindico.io
      - name: Notify
...
- Workflow separado `deploy-rc.yml` envia pra ambiente `rc.sistema-sindico.io`.
