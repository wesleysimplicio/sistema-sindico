...
Adote git-flow apenas se o projeto exige releases longas com hardening, ex: software embarcado, app mobile com review de loja, produto SaaS com janela de manutenção rigorosa. Branches: `main` (produção), `develop` (integração), `feature/*`, `release/*`, `hotfix/*`. Para `sistema-sindico` web/SaaS o padrão é trunk-based.
...
|----------|------------|---------|-----|
| `dev` | qualquer PR | abrir/atualizar PR | preview-<pr>.sistema-sindico.dev |
| `staging` | `main` | merge em main | staging.sistema-sindico.io |
| `production` | tag `vX.Y.Z` | tag push manual | sistema-sindico.io |
...
Configuração em GitHub do repo `sistema-sindico`:
